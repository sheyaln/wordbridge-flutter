...
Counts are measured at commit `91df220`. Levels are code and they move; the
...
At `91df220`: **372 locations, 366 distinct labels.** Six labels hold a
...
Of the 372 locations, 363 speak a word. Eight are grammar keys
(`+s`, `+ed`, `+ing`, `+'s`, `am/is/are`, `was/were`, `a`, `the`) and one is a
punctuation mark (`?`).
...
|---|---|---|---|
| root board, content area | 36 | 58 | 66 |
...
|---|---|---|---|---|
| Under 6 | 1 | 99 | 241 | 372 |
| 6 to 12 | 2 | 99 | 241 | 372 |
| 13 to 17 | 2 | 108 | 262 | 399 |
| 18 and over | 2 | 112 | 271 | 404 |
...
Measured at `91df220`, the level-1 root board is **exactly** the Universal Core
...
| Punctuation in the pinned column (`?`) | 1 |
...
⚠️ **Unverified:** the Fitzgerald Key is conventionally dated to Edith
Fitzgerald's *Straight Language for the Deaf* (1926), which is the basis for
calling it out of copyright. Library catalogues reachable from here (Open
Library) hold the work under Edith Mansford Fitzgerald, Volta Bureau, with
editions from 1949 onward; the 1926 first publication was **not** independently
confirmed in this pass. The out-of-copyright claim in `core_vocabulary.dart`
therefore rests on a date this repo has not checked. The dependency is small —
what is used is a word-class order, not text — but the claim should be
corrected or confirmed rather than repeated.
...
The five root-board additions are spelled out rather than counted, because
...
`?` is appended to the pinned column rather than slotted among the question
words, because each of those already has a location somebody has learned.
...
of `91df220`; the counting rule is what does not go stale.
