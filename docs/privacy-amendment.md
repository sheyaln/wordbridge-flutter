...
| | |
|---|---|
| Where a copy goes, iOS/macOS | the app's own iCloud container, `Documents`, in the user's iCloud account |
...
| Who chooses | asked on the **first run only**, per device, with "keep a copy" preselected |
...
<ul>
  <li>On iPad and iPhone, to Wordbridge's own folder in your iCloud Drive, using the iCloud account signed in on the device.</li>
...
<p><strong>We never receive these copies and cannot read them.</strong> There is no Wordbridge server involved and no Wordbridge account. Wordbridge holds no credentials for your iCloud or Google account: on iPad the operating system grants the app a private folder inside your own iCloud storage, and on Android you grant permission to one folder you picked yourself. The copies count against your own iCloud or Drive storage and are governed by Apple's or Google's privacy policy for that account, not by ours.</p>
...
- **Data collection answers: no change.** Apple defines collection as
  ...
  profiles.
- **App Review Notes: must be updated.** The build now carries the iCloud
...
- **Capability provisioning.** iCloud Documents requires a paid account and the
  capability on the provisioning profile. See `docs/deploy-ipad.md`.
...
- **No new permissions in the manifest.** The document picker needs none, which
  ...
  permissions cross-check will show nothing new.
